...
class NCFileDataset(Dataset):
    ...
    def __init__(
        self,
        data_glob="regression-data/??/all/gdas.f06+inc.202[0-4]??.nc",
        transform=None,
    ...
    def __getitems__(self, flat_indexes: list[int]):
        ds = self.ds
...
def train_iteration(loader, model, loss_fn, optim):
...
def test_iteration(loader, model, loss_fn):
...
def save_checkpoint(model, optimizer, scheduler, epoch, path, mlflow_enabled=False):
...
def log_mlflow_artifact(path, artifact_path=None, mlflow_enabled=False):
    if mlflow_enabled:
        import mlflow
...
if __name__ == "__main__":
    ...
    with pytorch_distributed() as (rank, world_size, local_rank):
        ...
        train_dataset = NCFileDataset(
            "regression-data/??/all/gdas.f06+inc.202[0-4]??.nc"
        )
        ...
        test_dataset = NCFileDataset(
            "regression-data/??/all/gdas.f06+inc.20[12][95]??.nc"
        )
